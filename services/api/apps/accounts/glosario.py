...
**Los textos de los términos van sin `**negritas**`**: esto no se renderiza como Markdown sino como
texto en una plantilla de Django, y los asteriscos salen tal cual en pantalla. Se vio mirándola.
...
## El texto va en español literal, sin `gettext`

Por el mismo motivo que `ayuda.py`, y ahí está escrito: el catálogo va de `msgid` en inglés a
`msgstr` en español, así que marcar prosa que ya está en español obliga a traducirla a sí misma.
"""
...
#: Los tres grupos, en el orden en que se encuentra uno con ellos: primero cómo se organiza el
#: trabajo, después qué es el modelo, y al final qué se hace con él. No alfabético a propósito —un
#: índice alfabético no enseña que el CDE y el BEP son la misma conversación.
...
class Termino:
    ...
    sigla: str
    #: Lo que la sigla abrevia. Vacío cuando el término ya es su nombre («Gemelo digital»).
    nombre: str
    que_es: str
...
def terminos_por_grupo() -> list[tuple[str, list[TerminoResuelto]]]:
    """El vocabulario agrupado, en el orden de `GRUPOS`.
    ...
    por_grupo: dict[str, list[TerminoResuelto]] = {grupo: [] for grupo in GRUPOS}
    ...
    return [(grupo, por_grupo[grupo]) for grupo in GRUPOS]


def cuantos_no_estan() -> int:
    """Cuántos de los dieciséis no están en el producto, para poder decirlo arriba."""
    return sum(1 for uno in TERMINOS if not uno.lo_hace)
